...
A lightweight RESTful CRUD API built with Node.js, Express.js, and SQLite. This API allows users to manage a to-do list with full support for creating, reading, updating, and deleting tasks, alongside query filtering, title search, and live task metrics. The API uses SQLite for persistent data storage, so tasks survive server restarts. The repository contains the hand-built Express.js To Do API in the root files and the quarantined AI-generated version in [`ai-version/`](ai-version/).
...
**SQLite** is a lightweight, serverless database that stores data in a **single file**. It requires **no separate database server installation or configuration**, making it perfect for this project while effectively demonstrating how **backend persistence** works.

---

## 📁 **Where the Database File Is Stored**

The database is stored locally in a single file named **`tasks.db`**, which is located directly at the **root of the project directory** alongside `index.mjs`.
...
During manual database exploration in a **database viewer**, the following query was executed to count all tasks currently stored in the database:
...
## 🖥️ **Database Viewer Screenshot**

Here is a snapshot of the database running inside the database viewer:

![Database Viewer Screenshot](./Images/db.png)

---

# 📌 **API Endpoints**

The API provides a complete set of **RESTful CRUD endpoints** for managing tasks, alongside **health checks, filtering, searching, and live task metrics**.
...
| **GET** | **`/tasks`** | List all tasks. Supports **`?done=true`** and **`?search=term`** | `200` |
